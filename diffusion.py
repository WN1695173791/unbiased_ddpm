...
class GaussianDiffusion(nn.Module):
    ...
    def p_mean_variance(self, model, x, t, clip_denoised, roll_pix):
        x_recon = self.predict_start_from_noise(x, t, noise=model(x, t, roll_pix))
    ...
    def p_sample(self, model, x, t, noise_fn, clip_denoised=True, repeat_noise=False, roll_pix=None):
        mean, _, log_var = self.p_mean_variance(model, x, t, clip_denoised, roll_pix)
    ...
    @torch.no_grad()
    def p_sample_loop(self, model, shape, device, noise_fn=torch.randn, roll_pix=None):
        ...
        for i in reversed(range(self.num_timesteps)):
            ...
            if i == 100:
                img = torch.roll(img, shape[2] // 2, 2)

        return img

    @torch.no_grad()
    def p_recon_loop(self, model, shape, device, t=1000, ref=None, noise_fn=torch.randn, roll_pix=None):
        #img = noise_fn(shape, device=device)
        time = torch.full((shape[0],), t, dtype=torch.int64).to(device)
        img = self.q_sample(ref, time, noise_fn(shape, device=device))
